...
struct Node {
    ...
    Node(std::size_t next, Quad quad) : children(0), next_(next), pos(vec2(0, 0)), mass(0.0), quad_(quad) {}
...
private:
    float t_sq; /* theta squared, opening angle criterion.
                    This theta allows us to determine how "far" is far enough
                    to classify a bunch of distance bodies as a single body (via the center of mass) */
    float e_sq; /* epsilon squared, gravitational softening
                    This epsilon value it prevents the force from blowing up when two masses get extremely close together
                    Real gravity is F ∝ 1/r², so as r → 0, force → infinity
                    In discrete timestep sim, two close particles will produce an overestimated force
                    Epsilon smoothly caps out this force */
    const size_t ROOT = 0;
...
public:
    ...
    QuadTree() : nodes_(), parents_() {}
    QuadTree(float theta, float epsilon) : t_sq(theta * theta), e_sq(epsilon * epsilon), nodes_(), parents_() {};
    ...
    std::size_t subdivide(std::size_t node) {
        ...
        std::vector<size_t> nexts = {children + 1,
                children + 2,
                children + 3,
                nodes_[node].next_};
    ...
    vec2 acc(vec2 pos) {
        ...
        while (true) {
            ...
            // if this node's spatial extent, divided by its distance from the query point,
            // is smaller than θ, treat it as a single mass instead of descending into its children
